// This file was removed as of 2019-04-19. We should get rid of it as a dependency. But for now just copy/paste it here
    ...
    namespace spirit
    {
        namespace x3
        {
            namespace testing
            {
                ...
                template <typename Iterator> struct compare_result
                {
                    compare_result(Iterator pos, bool full_match) : pos(pos), full_match(full_match) {}
                ...
                ////////////////////////////////////////////////////////////////////////////
                template <typename F>
                bool compare(fs::path input_path, fs::path expect_path, F f, char const* re_prefix = "<%",
                ...
                ////////////////////////////////////////////////////////////////////////////
                template <typename F> int for_each_file(fs::path p, F f);
                ...
                inline bool is_regex(Iterator& first, Iterator last, std::string& re, char const* re_prefix,
                                     char const* re_suffix)
                {
                ...
                compare(Range const& in, Range const& tem, char const* re_prefix, char const* re_suffix)
                {
                    ...
                    while (in_first != in_last && tem_first != tem_last)
                    {
                        if (is_regex(tem_first, tem_last, re, re_prefix, re_suffix))
                        {
                            ...
                            boost::regex e(re);
                            if (!boost::regex_search(in_first, in_last, what, e,
                ...
                template <typename F> inline int for_each_file(fs::path p, F f)
                {
                    try
                    {
                ...
                template <typename F>
                inline bool compare(fs::path input_path, fs::path expect_path, F f, char const* re_prefix,
                                    char const* re_suffix)
                {
                    ...
                    if (!result.full_match)
                    {
                        ...
                        std::cerr << "==== File: " << expect_path << ", Line: " << line << ", Column: " << col
                                  << std::endl;
                        std::cerr << "=============================================" << std::endl;
