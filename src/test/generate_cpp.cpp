...
auto generate = [](std::string const& source, fs::path inputPath) {
    ...
    if (success)
    {
        ...
        flatmessage::generator::cpp_generator generator;
...
DEF_TEST(GenerateInputFiles, generate_cpp)
{
    auto compare
        = [](fs::path inputPath, fs::path expectPath) { return testing::compare(inputPath, expectPath, generate); };
